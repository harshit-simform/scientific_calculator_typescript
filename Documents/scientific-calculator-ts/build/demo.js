...
function handleEvents(btn) {
    ...
    switch (btnText) {
        ...
        case "F-E":
            ...
            operation = `${Number.parseFloat(data.operation.join("")).toExponential()}`;
            emptyarray();
            break;
        default:
            btnText = "";
...
function memoryFunction(event) {
    const targetElement = event.target;
    const operation = targetElement.innerHTML;
    switch (operation) {
        case "MS":
    ...
    if (value == "trigoToggle") {
        trigoButton.forEach((btn) => {
            ...
            if (toggleFlag.inverseFunction) {
                ...
                btnTextReplacement =
                    (_a = Object.keys(trigoObject).find((key) => trigoObject[key][1] === btnText)) !== null && _a !== void 0 ? _a : btnText;
            }
        ...
        trigoButton.forEach((btn) => {
            ...
            if (toggleFlag.hyperFunction) {
                ...
                btnTextReplacement =
                    (_a = Object.keys(trigoObject).find((key) => trigoObject[key][2] === btnText)) !== null && _a !== void 0 ? _a : btnText;
            }
            btn.innerHTML = btnTextReplacement;
        });
        ...
        const trigoFunction = document.getElementById("trigo_function");
